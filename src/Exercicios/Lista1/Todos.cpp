...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Todos Triângulos! -- Enzo", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Todos Triângulos! -- Enzo\tFPS %.2lf", fps);
        ...
        // Triângulo só contorno (centro)
        glLineWidth(4);
        glDrawArrays(GL_LINE_LOOP, 3, 3);

        // Triângulo só pontos (direita)
