...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Círculo! -- Enzo", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Círculo! -- Enzo\tFPS %.2lf", fps);
		...
		glUniform4f(colorLoc, 0.0f, 0.7f, 1.0f, 1.0f); // cor do círculo
		glLineWidth(3);
		glDrawArrays(GL_LINE_LOOP, 0, 101); // 101 = N+1, para fechar o círculo
...
int setupGeometry()
{
    const int N = 100; // Número de segmentos (quanto maior, mais suave)
    const float PI = 3.14159265358979323846f;
    float raio = 0.5f;
    float centro_x = 0.0f, centro_y = 0.0f;

    // +1 para fechar o círculo
