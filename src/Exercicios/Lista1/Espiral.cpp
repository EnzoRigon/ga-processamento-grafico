...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Espiral! -- Enzo", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Espiral! -- Enzo\tFPS %.2lf", fps);
		...
		glUniform4f(colorLoc, 1.0f, 0.7f, 0.3f, 1.0f); // cor da espiral
		glDrawArrays(GL_LINE_STRIP, 0, 500); // 500 = N
...
int setupGeometry()
{
    const int N = 500; // Número de pontos da espiral
    const float PI = 3.14159265358979323846f;
    float centro_x = 0.0f, centro_y = 0.0f;
    float a = 0.02f; // controla o afastamento inicial
    float b = 0.04f; // controla o "espaçamento" entre as voltas

    GLfloat vertices[N*3];

    for (int i = 0; i < N; ++i) {
        float t = 4.0f * PI * i / (N-1); // 2 voltas completas
        float r = a + b * t;
        float x = centro_x + r * cos(t);
        float y = centro_y + r * sin(t);
        vertices[i*3 + 0] = x;
        vertices[i*3 + 1] = y;
        vertices[i*3 + 2] = 0.0f;
    }
