...
// Fragment Shader: aceita cor por atributo ou uniforme
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Exe 8! -- Enzo", nullptr, nullptr);
    if (!window)
    {
    ...
    // Cria VAO/VBO com 3 vértices coloridos
    GLuint VAO = setupGeometry();

    glUseProgram(shaderID);

    // Uniforms para cor da linha
    GLint useUniformColorLoc = glGetUniformLocation(shaderID, "useUniformColor");
    GLint uniformColorLoc = glGetUniformLocation(shaderID, "uniformColor");
    ...
    while (!glfwWindowShouldClose(window))
    {
        // FPS na barra de título
        {
            ...
            if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
            {
                ...
                char tmp[256];
                sprintf(tmp, "Exe 8! -- Enzo\tFPS %.2lf", fps);
        ...
        // Desenha 3 pontos, cada um com sua cor
        glUniform1i(useUniformColorLoc, 0); // usa cor do atributo
        glDrawArrays(GL_POINTS, 0, 3);

        // Desenha linha azul conectando os pontos
        glUniform1i(useUniformColorLoc, 1); // usa cor uniforme
        glUniform3f(uniformColorLoc, 0.0f, 0.0f, 1.0f); // azul
        glLineWidth(3.0f);
...
// Cria VAO/VBO com 3 vértices, cada um com uma cor diferente
int setupGeometry()
{
    GLfloat vertices[] = {
        //  posição         // cor
