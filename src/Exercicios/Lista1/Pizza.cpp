...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Pizza! -- Enzo", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Pizza! -- Enzo\tFPS %.2lf", fps);
		...
		glUniform4f(colorLoc, 1.0f, 0.9f, 0.3f, 1.0f); // cor de pizza (amarelo claro)
...
int setupGeometry()
{
    const int N = 50; // Número de segmentos para suavidade da borda da fatia
    const float PI = 3.14159265358979323846f;
    float raio = 0.5f;
    float centro_x = 0.0f, centro_y = 0.0f;

    // Ângulo da fatia da pizza (em radianos)
    float angulo_fatia = PI / 4.0f; // 45 graus de fatia

    // Vetor de vértices: centro + arco do setor
    ...
    int idx = 1;
    for (int i = 0; i <= N; ++i) {
        float theta = -angulo_fatia/2 + angulo_fatia * i / N; // setor centrado no eixo X
